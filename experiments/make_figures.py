...
R = str(pathlib.Path(__file__).resolve().parent.parent / "results")
...
ax.plot(
    N, [r["frame_ms"]["mean"] for r in e1], "o-", ms=3.5, label="mean", color="#16243F"
)
ax.plot(
    N, [r["frame_ms"]["p99"] for r in e1], "s--", ms=3.5, label="p99", color="#C9A24A"
)
ax.plot(
    N, [r["frame_ms"]["max"] for r in e1], "^:", ms=3.5, label="max", color="#8a1f1f"
)
...
fig.tight_layout()
fig.savefig(f"{R}/fig_scalability.pdf")
fig.savefig(f"{R}/fig_scalability.png")

# Fig B: telemetry latency and stale updates vs fleet size
fig, ax = plt.subplots(figsize=(3.45, 2.1))
ax.plot(
    N,
    [r["telemetry_lat_ms"]["p50"] for r in e1],
    "o-",
    ms=3.5,
    label="telemetry p50",
    color="#16243F",
...
ax.plot(
    N,
    [r["telemetry_lat_ms"]["p99"] for r in e1],
    "s--",
    ms=3.5,
    label="telemetry p99",
    color="#C9A24A",
...
fig.tight_layout()
fig.savefig(f"{R}/fig_latency.pdf")
fig.savefig(f"{R}/fig_latency.png")
...
axs[0].bar(
    ["ON\n(8 Hz)", "OFF\n(50 Hz)"],
    [on["uplink_Bps"] / 1024, off["uplink_Bps"] / 1024],
    color=["#16243F", "#C9A24A"],
    width=0.55,
)
axs[0].set_ylabel("Uplink usage (KiB/s)")
axs[0].set_title("Bandwidth", fontsize=8)
axs[1].bar(
    ["ON\n(8 Hz)", "OFF\n(50 Hz)"],
    [on["dup_updates"], off["dup_updates"]],
    color=["#16243F", "#C9A24A"],
    width=0.55,
)
axs[1].set_ylabel("Redundant updates")
axs[1].set_title("Wasted samples", fontsize=8)
for a in axs:
    for c in a.containers:
        a.bar_label(c, fontsize=7, fmt="%.0f")
    a.margins(y=0.18)
fig.tight_layout()
fig.savefig(f"{R}/fig_regulator.pdf")
fig.savefig(f"{R}/fig_regulator.png")
print("figures ok")
...
if os.path.exists(f"{_R}/e3_swarm.json"):
    ...
    for r, (ls, c) in zip(e3, styles):
        ...
        ax.plot(
            ...
            color=c,
            lw=1.3,
            label=f"N={r['n_agents']} ({r['swarm_lat_ms']['n']} events)",
        )
    ...
    ax.legend(frameon=False, fontsize=7, loc="center right")
    fig.tight_layout()
    fig.savefig(f"{_R}/fig_swarm.pdf")
    fig.savefig(f"{_R}/fig_swarm.png")
    print("fig_swarm ok")
